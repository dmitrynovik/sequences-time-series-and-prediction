...
print(tf.keras.losses.MSE(x_valid, naive_forecast).numpy())
print(tf.keras.losses.MAE(x_valid, naive_forecast).numpy())
# Expected Output
# 19.578304
# 2.6011968
...
def moving_average_forecast(series, window_size):
  ...
  result = []
  for offset in range(0, len(series) - window_size):
    avg = series[offset : offset + window_size].mean()
    result.append(avg)
...
print(tf.keras.losses.MSE(x_valid, moving_avg).numpy())
print(tf.keras.losses.MAE(x_valid, moving_avg).numpy())
# EXPECTED OUTPUT
# 65.786224
# 4.3040023
...
# diff_moving_avg = moving_average_forecast(series, 50)[split_time - 365 - 50:]
...
print(tf.keras.losses.MSE(x_valid, diff_moving_avg_plus_past).numpy())
print(tf.keras.losses.MAE(x_valid, diff_moving_avg_plus_past).numpy())
# EXPECTED OUTPUT
# 8.498155
# 2.327179
...
print(tf.keras.losses.MSE(x_valid, diff_moving_avg_plus_smooth_past).numpy())
print(tf.keras.losses.MAE(x_valid, diff_moving_avg_plus_smooth_past).numpy())
# EXPECTED OUTPUT
# 12.527958
# 2.2034433